...
class ExperimentConfig:
    ...
    # Uncertainty handling for rPINN
    # The updated default is to use the learned sigma/noise model for both
    # the data term and the physics term. The constant sigmas are retained only
    # as a backward-compatible fallback when no sigma model is supplied.
    use_learned_sigma_rpinn: bool = True
    sigma_eval_mode: str = "predicted_state"   # {"predicted_state", "observed_state"}
    sigma_detach_state: bool = True
    sigma_floor_rpinn: float = 1e-4

    # Backward-compatible fallback uncertainties
