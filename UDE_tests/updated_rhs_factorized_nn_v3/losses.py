...
def randomized_residual_nll(resid, sigma, xi):
    sigma = torch.as_tensor(sigma, dtype=resid.dtype, device=resid.device)
    sigma = torch.clamp(sigma, min=1e-8)
    return 0.5 * torch.mean(((resid - sigma * xi) / sigma) ** 2)
