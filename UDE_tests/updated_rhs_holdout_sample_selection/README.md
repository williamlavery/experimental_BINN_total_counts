...
- `training.py` - rPINN training, holdout-based checkpoint selection, and sigma/rho fitting
...
- rPINN now supports a train/holdout split over the observed trajectory points.
- Each randomized run is trained on the training observations only, but its checkpoint can be selected by the lowest holdout trajectory MSE.
- After all runs, posterior summaries can be formed from either all samples or just the best holdout-ranked subset via `n_selected_samples`.

## New outputs from `fit_multi_ic_rpinn(...)`

- Holdout split metadata:
  - `train_idx_obs`, `val_idx_obs`, `t_obs_train`, `t_obs_val`, `use_holdout`
- Per-sample validation diagnostics:
  - `val_data_histories`, `holdout_mses`, `best_selection_metrics`, `selected_sample_indices`
- Selected-sample summaries:
  - `selected_traj_histories`, `selected_obs_fit_histories`, `selected_obs_val_fit_histories`
  - `selected_h_histories`, `selected_rhs_histories`
- Structural diagnostics for both objects:
  - learned / MAP `H(N)` via `map_h`, `h_histories`, `selected_h_histories`, `h_mean`, `h_lo`, `h_hi`
  - learned / MAP `G(N)` via `map_rhs`, `rhs_histories`, `selected_rhs_histories`, `rhs_mean`, `rhs_lo`, `rhs_hi`

## Notes

- The old `fit_multi_ic_pinn(...)` entry point remains as a backward-compatible alias to the rPINN trainer.
- If `val_fraction_rpinn > 0` and `select_by_holdout=True`, randomized runs are checkpointed by holdout trajectory MSE; the deterministic MAP run still uses the training objective.
